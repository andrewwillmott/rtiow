...
//          "Ray-Tracing in One Weekend", taking a more data-oriented approach,
//          whilet still casting one ray at a time.
//
...
Vec3f Trace(tRNG& rng, const PD3f& r, const cScene& scene, int depth)
{
    PD3f hit;
    int hitIndex = Intersect(scene, r, hit);
    ...
    if (hitIndex >= 0)
    {
        ...
        if (md.mKind == kMaterialDiffuse && Scatter(rng, scene.mDiffuseMaterials[md.mIndex], r, hit, attenuation, scattered))
             return attenuation * Trace(rng, scattered, scene, depth - 1);

        if (md.mKind == kMaterialMetal   && Scatter(rng, scene.mMetalMaterials  [md.mIndex], r, hit, attenuation, scattered))
             return attenuation * Trace(rng, scattered, scene, depth - 1);

        if (md.mKind == kMaterialGlass   && Scatter(rng, scene.mGlassMaterials  [md.mIndex], r, hit, attenuation, scattered))
             return attenuation * Trace(rng, scattered, scene, depth - 1);
    }
...
void TraceScene(tRNG& rng, int nx, int ny, int ns, int nd, const cCamera& camera, const cScene& scene)
{
    float ws = 1.0f / ns;
    ...
    for (int j = ny - 1; j >= 0; j--)
        for (int i = 0; i < nx; i++)
        {
            ...
            for (int s = 0; s < ns; s++)
            {
                ...
                pixel += Trace(rng, r, scene, nd);
            }

            pixel *= w;
            pixel = Vec3f(sqrtf(pixel.x), sqrtf(pixel.y), sqrtf(pixel.z));    // rough gamma
            
            int ir = int(255 * pixel.x);
            int ig = int(255 * pixel.y);
            int ib = int(255 * pixel.z);
